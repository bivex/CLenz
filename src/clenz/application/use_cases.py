...
from clenz.application.dto import (
...
from clenz.domain.events import (
    ParsingJobCompleted,
    ParsingJobStarted,
    SourceUnitParsed,
    SourceUnitParsingFailed,
)
from clenz.domain.model import ParseOutcome, ParseStatus, ParsingJob, SourceUnit
from clenz.domain.ports import (
    CSyntaxParser,
    Clock,
...
class ParsingJobService:
    ...
    def parse_directory(self, command: ParseDirectoryCommand) -> ParsingJobReportDTO:
        source_units = tuple(self.source_repository.list_c_sources(command.root_path))
